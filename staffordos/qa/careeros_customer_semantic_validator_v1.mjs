...
const checks = {
  ...
  capabilities_remain_confirmed_fact_only: /CUSTOMER_CONFIRMED_SOURCE_BACKED/.test(product) && /deriveCapabilityCandidates\(facts\)/.test(product) && /supportingEvidence/.test(product),
  capability_review_exposes_confirmed_evidence: /Why CareerOS is asking/.test(capabilities) && /supportingEvidence/.test(capabilities) && /sourceExcerpt/.test(product),
