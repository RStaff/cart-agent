...
function exercise007CertificationMemo({
  exercise = "Exercise 007 - Header Navigation Inventory",
  canonicalStore = "no-kings-athletics.myshopify.com",
  decision = "CONDITIONAL GO"
} = {}) {
  return `# Mission 001 Exercise 007 Certification
...
- Header group entry point: \`sections/header-group.json\`
- Header section: \`sections/header.liquid\`
- Desktop navigation, mobile drawer, header actions, search, predictive search, and mega-menu architecture were certified from repository-backed evidence.
...
- Exercise 007 mission proof package
...
## Readiness Assessment
- Current phase before certification: \`mission_certification\`
- Next phase after certification: \`exercise_008_planning\`
...
function writeFixture({
  ...
  const activeExerciseSlug = /Exercise 004 - Product Page Inventory/i.test(activeExercise)
    ...
    : /Exercise 005 - Collection Page Inventory/i.test(activeExercise)
      ? "exercise_005"
      : /Exercise 006 - Cart Inventory/i.test(activeExercise)
        ? "exercise_006"
        : /Exercise 007 - Header Navigation Inventory/i.test(activeExercise)
          ? "exercise_007"
          : /Exercise 008 - Trust Badge Inventory/i.test(activeExercise)
            ? "exercise_008"
          : "";
  ...
  fs.mkdirSync(path.join(exercisesDir, "exercise_006"), { recursive: true });
  fs.mkdirSync(path.join(exercisesDir, "exercise_007"), { recursive: true });
  fs.mkdirSync(path.join(exercisesDir, "exercise_008"), { recursive: true });
  ...
  if (scope006 !== null) {
    writeText(path.join(exercisesDir, "exercise_006/fix_scope.md"), scope006);
  }
  if (scope007 !== null) {
    writeText(path.join(exercisesDir, "exercise_007/fix_scope.md"), scope007);
  }
  if (scope008 !== null) {
    writeText(path.join(exercisesDir, "exercise_008/fix_scope.md"), scope008);
  }
  if (includeBefore) {
...
function runReadiness(root) {
  ...
  return evaluateNokingsMissionReadiness({
    ...
    exercise007CertificationMemoPath: path.join(root, "staffordos/implementation/p11_21_mission_001_exercise_007_certification_v1.md")
  });
}
...
function run() {
  ...
  writeText(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_006/fix_scope.md"), scopeContent({
    exercise: "Exercise 006 - Cart Inventory",
    objective: "The governed training objective is to inventory the NoKings cart files that affect trust, totals, and checkout readiness.",
    targetArtifact: "templates/cart.json\nsections/main-cart.liquid\nsnippets/cart-summary.liquid\nsnippets/cart-items-component.liquid",
    inScope: ["Cart page file-stack inventory", "Cart summary and item-rendering responsibility mapping", "Checkout-adjacent source mapping"],
    outOfScope: ["Shopify theme edits", "Payment changes", "Completion changes"]
  }));
  const exercise006ScopeReport = evaluateNokingsMissionReadiness({
    repoRoot: REPO_ROOT,
    bindingPath: path.join(exercise005Root, "staffordos/missions/mission_001_nokings_shopifixer_binding_v1.json"),
    proofRunDir: path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1"),
    certificationMemoPath: path.join(exercise005Root, "staffordos/implementation/p10_9_mission_001_exercise_004_certification_v1.md"),
  ...
  assert(exercise006ScopeReport.active_exercise === "Exercise 006 - Cart Inventory", "valid exercise 006 scope becomes the active exercise", failures);
  assert(exercise006ScopeReport.current_phase === "before_evidence", "valid exercise 006 scope advances to before_evidence", failures);
  assert(exercise006ScopeReport.current_blocker === "Before Evidence Missing", "valid exercise 006 scope exposes the before evidence blocker", failures);
  assert(exercise006ScopeReport.next_safe_action === "Capture Before Evidence", "valid exercise 006 scope next action is capture before evidence", failures);
  assert(exercise006ScopeReport.gates.scope.status === "pass", "exercise 006 scope gate passes from its own file", failures);
  assert(exercise006ScopeReport.gates.before_evidence.status === "blocked", "exercise 006 before evidence remains blocked until captured", failures);
  ...
  writeText(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_006/execution_notes.md"), `# Execution Notes\n\nStatus:\nComplete\n\nMission:\nMission 001 - NoKings Shopify Engineering Training\n\nExercise:\nExercise 006 - Cart Inventory\n\nStore:\nno-kings-athletics.myshopify.com\n\nObjective:\nCart inventory from repository-backed source files.\n\nAffected Page / Artifact:\nCart inventory\n\nNotes:\n- Cart template, cart summary, cart item rendering, quantity update, item removal, checkout CTA, and cart drawer source references inventoried.\n- No Shopify mutation occurred.\n`);
  const completedCartInventoryReport = evaluateNokingsMissionReadiness({
    repoRoot: REPO_ROOT,
    bindingPath: path.join(exercise005Root, "staffordos/missions/mission_001_nokings_shopifixer_binding_v1.json"),
    proofRunDir: path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1"),
    certificationMemoPath: path.join(exercise005Root, "staffordos/implementation/p10_9_mission_001_exercise_004_certification_v1.md"),
  ...
  assert(completedCartInventoryReport.current_phase === "after_evidence", "completed exercise 006 cart inventory advances to after_evidence", failures);
  assert(completedCartInventoryReport.current_blocker === "After Evidence Missing", "completed exercise 006 cart inventory exposes the after evidence blocker", failures);
  assert(completedCartInventoryReport.next_safe_action === "Capture After Evidence", "completed exercise 006 cart inventory next action is after evidence", failures);
  assert(completedCartInventoryReport.gates.execution.status === "pass", "completed exercise 006 cart inventory passes execution gate", failures);
  assert(completedCartInventoryReport.gates.after_evidence.status === "blocked", "exercise 006 after evidence remains blocked after inventory", failures);
  writeText(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_006/after_evidence.md"), `# After Evidence\n\nStatus:\nComplete\n\nMission:\nMission 001 - NoKings Shopify Engineering Training\n\nExercise:\nExercise 006 - Cart Inventory\n\nStore:\nno-kings-athletics.myshopify.com\n\nAffected Page / Artifact:\nCart inventory completion evidence\n\nIssue:\nCart inventory completed from repository-backed Exercise 006 artifacts.\n\nObserved Improvement:\nNot Claimed\n\nMerchant-Facing Summary:\nNot Claimed\n\nRemaining Limitations:\n- Runtime cart behavior is Not Yet Proven.\n\nScreenshot:\nNot Yet Available\n\nNotes:\n- Cart after evidence only.\n- No Shopify mutation occurred.\n`);
  const completedCartAfterEvidenceReport = evaluateNokingsMissionReadiness({
    repoRoot: REPO_ROOT,
    bindingPath: path.join(exercise005Root, "staffordos/missions/mission_001_nokings_shopifixer_binding_v1.json"),
    proofRunDir: path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1"),
    certificationMemoPath: path.join(exercise005Root, "staffordos/implementation/p10_9_mission_001_exercise_004_certification_v1.md"),
  ...
  assert(completedCartAfterEvidenceReport.current_phase === "proof_package", "completed exercise 006 after evidence advances to proof_package", failures);
  assert(completedCartAfterEvidenceReport.current_blocker === "Proof Package Missing", "completed exercise 006 after evidence exposes the proof package blocker", failures);
  ...
  writeText(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_007/fix_scope.md"), scopeContent({
    exercise: "Exercise 007 - Header Navigation Inventory",
    objective: "The governed training objective is to inventory the NoKings header and mobile navigation control points.",
    targetArtifact: "sections/header-group.json\nsections/header.liquid\nsections/header-announcements.liquid\nsnippets/header-row.liquid\nsnippets/header-drawer.liquid\nassets/header-menu.js",
    inScope: ["Header group file-stack inventory", "Desktop navigation ownership mapping", "Mobile drawer navigation ownership mapping"],
    outOfScope: ["Shopify theme edits", "Payment changes", "Completion changes"]
  }));
  const exercise007ScopeReport = evaluateNokingsMissionReadiness({
    repoRoot: REPO_ROOT,
    bindingPath: path.join(exercise005Root, "staffordos/missions/mission_001_nokings_shopifixer_binding_v1.json"),
    proofRunDir: path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1"),
    certificationMemoPath: path.join(exercise005Root, "staffordos/implementation/p10_9_mission_001_exercise_004_certification_v1.md"),
  ...
  assert(exercise007ScopeReport.active_exercise === "Exercise 007 - Header Navigation Inventory", "valid exercise 007 scope becomes the active exercise", failures);
  assert(exercise007ScopeReport.current_phase === "before_evidence", "valid exercise 007 scope advances to before_evidence", failures);
  assert(exercise007ScopeReport.current_blocker === "Before Evidence Missing", "valid exercise 007 scope exposes the before evidence blocker", failures);
  assert(exercise007ScopeReport.next_safe_action === "Capture Before Evidence", "valid exercise 007 scope next action is capture before evidence", failures);
  assert(exercise007ScopeReport.gates.scope.status === "pass", "exercise 007 scope gate passes from its own file", failures);
  assert(exercise007ScopeReport.gates.before_evidence.status === "blocked", "exercise 007 before evidence remains blocked until captured", failures);
  ...
  writeText(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_007/execution_notes.md"), `# Execution Notes\n\nStatus:\nComplete\n\nMission:\nMission 001 - NoKings Shopify Engineering Training\n\nExercise:\nExercise 007 - Header Navigation Inventory\n\nStore:\nno-kings-athletics.myshopify.com\n\nObjective:\nHeader navigation inventory from repository-backed source files.\n\nAffected Page / Artifact:\nHeader Navigation Inventory\n\nNotes:\n- Header group, header section, desktop navigation, mobile drawer, search, predictive search, announcement bar, and header actions were inventoried.\n- No Shopify mutation occurred.\n`);
  const exercise007InventoryReport = evaluateNokingsMissionReadiness({
    repoRoot: REPO_ROOT,
    bindingPath: path.join(exercise005Root, "staffordos/missions/mission_001_nokings_shopifixer_binding_v1.json"),
    proofRunDir: path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1"),
    certificationMemoPath: path.join(exercise005Root, "staffordos/implementation/p10_9_mission_001_exercise_004_certification_v1.md"),
  ...
  assert(exercise007InventoryReport.current_phase === "after_evidence", "valid exercise 007 inventory advances to after_evidence", failures);
  assert(exercise007InventoryReport.current_blocker === "After Evidence Missing", "valid exercise 007 inventory exposes the after evidence blocker", failures);
  ...
  writeText(path.join(exercise005Root, "staffordos/implementation/p11_21_mission_001_exercise_007_certification_v1.md"), exercise007CertificationMemo());
  const exercise007CertificationReport = evaluateNokingsMissionReadiness({
    repoRoot: REPO_ROOT,
    bindingPath: path.join(exercise005Root, "staffordos/missions/mission_001_nokings_shopifixer_binding_v1.json"),
    proofRunDir: path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1"),
    certificationMemoPath: path.join(exercise005Root, "staffordos/implementation/p10_9_mission_001_exercise_004_certification_v1.md"),
  ...
  assert(exercise007CertificationReport.current_phase === "exercise_008_planning", "valid exercise 007 certification advances to exercise_008_planning", failures);
  assert(exercise007CertificationReport.current_blocker === "Exercise 008 Planning Missing", "valid exercise 007 certification exposes exercise 008 planning blocker", failures);
  assert(exercise007CertificationReport.next_safe_action === "Plan Exercise 008 - Trust Badge Inventory", "valid exercise 007 certification next action is Exercise 008 planning", failures);
  assert(exercise007CertificationReport.gates.mission_certification.status === "pass", "mission certification gate passes when Exercise 007 certification is valid", failures);
  assert(exercise007CertificationReport.payment_required === false, "exercise 007 certification keeps payment not required", failures);
  assert(exercise007CertificationReport.completion_permitted === false, "exercise 007 certification keeps completion prohibited", failures);
  writeText(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_008/fix_scope.md"), scopeContent({
    exercise: "Exercise 008 - Trust Badge Inventory",
    objective: "The governed training objective is to inventory NoKings trust badge, payment icon, checkout-readiness, and CTA trust source architecture.",
    targetArtifact: "snippets/buy-buttons-styles.liquid\nblocks/payment-icons.liquid\nsnippets/product-badges-styles.liquid\nsnippets/button.liquid",
    inScope: ["Trust badge source inventory", "Payment icon source inventory", "CTA trust styling source inventory"],
    outOfScope: ["Shopify theme edits", "Payment changes", "Completion changes"]
  }));
  const exercise008ScopeReport = evaluateNokingsMissionReadiness({
    repoRoot: REPO_ROOT,
    bindingPath: path.join(exercise005Root, "staffordos/missions/mission_001_nokings_shopifixer_binding_v1.json"),
    proofRunDir: path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1"),
    certificationMemoPath: path.join(exercise005Root, "staffordos/implementation/p10_9_mission_001_exercise_004_certification_v1.md"),
  ...
  assert(exercise008ScopeReport.active_exercise === "Exercise 008 - Trust Badge Inventory", "valid exercise 008 scope becomes the active exercise", failures);
  assert(exercise008ScopeReport.current_phase === "before_evidence", "valid exercise 008 scope advances to before_evidence", failures);
  assert(exercise008ScopeReport.current_blocker === "Before Evidence Missing", "valid exercise 008 scope exposes the before evidence blocker", failures);
  assert(exercise008ScopeReport.next_safe_action === "Capture Before Evidence", "valid exercise 008 scope next action is capture before evidence", failures);
  assert(exercise008ScopeReport.gates.scope.status === "pass", "exercise 008 scope gate passes from its own file", failures);
  assert(exercise008ScopeReport.gates.before_evidence.status === "blocked", "exercise 008 before evidence remains blocked until captured", failures);
  ...
  writeText(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_008/execution_notes.md"), `# Execution Notes\n\nStatus:\nComplete\n\nMission:\nMission 001 - NoKings Shopify Engineering Training\n\nExercise:\nExercise 008 - Trust Badge Inventory\n\nStore:\nno-kings-athletics.myshopify.com\n\nAffected Page / Artifact:\nTrust Badge Inventory\n\nIssue:\nTrust badge, payment icon, checkout-readiness, shipping, returns, and CTA reassurance source inventory completed from repository-backed files.\n\nNotes:\n- Trust badge inventory source findings only.\n- No Shopify mutation occurred.\n`);
  const exercise008InventoryReport = evaluateNokingsMissionReadiness({
    repoRoot: REPO_ROOT,
    bindingPath: path.join(exercise005Root, "staffordos/missions/mission_001_nokings_shopifixer_binding_v1.json"),
    proofRunDir: path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1"),
    certificationMemoPath: path.join(exercise005Root, "staffordos/implementation/p10_9_mission_001_exercise_004_certification_v1.md"),
  ...
  assert(exercise008InventoryReport.current_phase === "after_evidence", "valid exercise 008 inventory advances to after_evidence", failures);
  assert(exercise008InventoryReport.current_blocker === "After Evidence Missing", "valid exercise 008 inventory exposes after evidence blocker", failures);
  assert(exercise008InventoryReport.next_safe_action === "Capture After Evidence", "valid exercise 008 inventory next action is after evidence", failures);
  assert(exercise008InventoryReport.gates.execution.status === "pass", "exercise 008 execution gate passes when trust badge inventory is valid", failures);
  assert(exercise008InventoryReport.gates.after_evidence.status === "blocked", "exercise 008 after evidence remains blocked until captured", failures);
  writeText(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_008/after_evidence.md"), `# After Evidence\n\nStatus:\nComplete\n\nMission:\nMission 001 - NoKings Shopify Engineering Training\n\nExercise:\nExercise 008 - Trust Badge Inventory\n\nStore:\nno-kings-athletics.myshopify.com\n\nObjective:\nTrust badge inventory completion evidence.\n\nAffected Page / Artifact:\nTrust Badge Inventory completion evidence\n\nIssue:\nTrust badge, payment icon, checkout-readiness, shipping, returns, and CTA reassurance inventory completed from repository-backed source files.\n\nObserved Improvement:\nNot Claimed\n\nMerchant-Facing Summary:\nNot Claimed\n\nRemaining Limitations:\nRuntime trust, payment, policy, app, metafield, and checkout behavior remain Not Yet Proven.\n\nScreenshot:\nNot Yet Proven\n\nNotes:\n- Trust badge after evidence only.\n- No Shopify mutation occurred.\n`);
  const exercise008AfterEvidenceReport = evaluateNokingsMissionReadiness({
    repoRoot: REPO_ROOT,
    bindingPath: path.join(exercise005Root, "staffordos/missions/mission_001_nokings_shopifixer_binding_v1.json"),
    proofRunDir: path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1"),
    certificationMemoPath: path.join(exercise005Root, "staffordos/implementation/p10_9_mission_001_exercise_004_certification_v1.md"),
  ...
  assert(exercise008AfterEvidenceReport.current_phase === "proof_package", "valid exercise 008 after evidence advances to proof_package", failures);
  assert(exercise008AfterEvidenceReport.current_blocker === "Proof Package Missing", "valid exercise 008 after evidence exposes proof package blocker", failures);
  assert(exercise008AfterEvidenceReport.next_safe_action === "Generate Exercise 008 Mission Proof Package", "valid exercise 008 after evidence next action is proof package generation", failures);
  assert(exercise008AfterEvidenceReport.gates.after_evidence.status === "pass", "exercise 008 after evidence gate passes when completion evidence is valid", failures);
  assert(exercise008AfterEvidenceReport.gates.proof.status === "blocked", "exercise 008 proof remains blocked until proof package exists", failures);
  fs.unlinkSync(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_008/after_evidence.md"));
  fs.unlinkSync(path.join(exercise005Root, "staffordos/proof_runs/mission_001_nokings_shopifixer_v1/exercises/exercise_008/execution_notes.md"));
  ...
  assert(actualReport.current_phase === "exercise_009_planning", "current phase is exercise_009_planning after Exercise 008 certification", failures);
  assert(actualReport.current_blocker === "Exercise 009 Planning Missing", "current blocker is Exercise 009 Planning Missing after Exercise 008 certification", failures);
  assert(actualReport.next_safe_action === "Plan Exercise 009 - Footer Inventory", "next safe action is Exercise 009 planning", failures);
  ...
  assert(actualReport.gates.execution.status === "pass", "trust badge inventory passes after Exercise 008 inventory is performed", failures);
  assert(actualReport.gates.after_evidence.status === "pass", "after evidence passes after Exercise 008 after evidence is captured", failures);
  assert(actualReport.gates.proof.status === "pass", "proof package passes after Exercise 008 proof package exists", failures);
  assert(actualReport.gates.mission_certification.status === "pass", "mission certification passes after Exercise 008 certification exists", failures);
