...
function run() {
  ...
  const lowSelfConfidenceRows = rows.filter((row) => row.review.selfConfidence !== "LOW");
  const lowSelfConfidenceLabels = { records: Object.fromEntries(lowSelfConfidenceRows.map((row) => [row.sampleId, row.review])) };
  const result = {
    ...
    diagnosticSelfConfidenceExclusion: { excludedCount: rows.length - lowSelfConfidenceRows.length, weightsChanged: false, groundTruthChanged: false, metrics: metricSet(lowSelfConfidenceRows, WEIGHT_SETS.EXPERIMENTAL_WEIGHT_SET_V1, humanRank(lowSelfConfidenceLabels), true) },
