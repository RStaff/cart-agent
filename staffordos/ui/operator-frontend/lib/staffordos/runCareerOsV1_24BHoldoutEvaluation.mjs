...
function buildRows(manifest, labels) {
  ...
  const rows = manifest.holdoutSet.map((item) => {
    ...
    const authorityDiagnostics = projectMatchAuthorityDiagnostics({ title: source.title || rec.role, requirements: fitArtifact?.requirements || [], mappings: fitArtifact?.mappings || [] });
...
function run() {
  ...
  const rows = buildRows(manifest, labels); const rerun = buildRows(manifest, labels);
...
function write(file, value) { writeFileSync(path.join(outputRoot, file), value); }
