...
export default async function CareerOnboardingPage() {
  ...
  return profile ? <main className="careerShell"><header className="careerHeader"><div><p className="careerEyebrow">CareerOS</p><h1>Build your career foundation</h1><p className="careerMuted">Signed in as {context.user.email}. Add source text when you are ready.</p></div></header><IntakeReview /></main> : <ProfileForm initialProfile={null} email={context.user.email} mode="onboarding" />;
}
