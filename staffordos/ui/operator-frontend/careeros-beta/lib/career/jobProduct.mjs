...
function classify(text) {
  ...
  if (/teach|train|workshop|instruction|facilitat|coach|curriculum|enablement|office hours/.test(value)) return { conceptKey: "TEACHING_TRAINING", scope: "training", specialist: false };
  ...
  if (/consult|client|customer|advisory|account engagement|implementation for clients|stakeholder-facing delivery/.test(value)) return { conceptKey: "CONSULTING_CLIENT_DELIVERY", scope: "client", specialist: false };
  if (/operations|workflow|process design|operational improvement|operational process|vendor coordination|business operations|process management|operational system|business ownership|entrepreneur/.test(value)) return { conceptKey: "BUSINESS_PROCESS_OPERATIONS", scope: "process", specialist: false };
