...
import { evaluateOpportunity, getOpportunity } from "../../../../../lib/career/careerP0Product.mjs";
...
export async function POST(request: Request, { params }: { params: Promise<{ opportunityId: string }> }) {
  ...
  const context = await currentCareerContext();
  if (!context) return NextResponse.json({ ok: false, error: "UNAUTHORIZED" }, { status: 401 });
  try { const { opportunityId } = await params; return NextResponse.json({ ok: true, match: await evaluateOpportunity(context, opportunityId) }); }
