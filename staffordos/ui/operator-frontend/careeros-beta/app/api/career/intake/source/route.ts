...
import { parseCareerText, sourceDigest, CAREEROS_INTAKE_EXTRACTOR_VERSION, CAREEROS_MAX_TEXT_LENGTH } from "../../../../../lib/career/careerP0Intake.mjs";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const digest = sourceDigest(text);
    const source = await careerP0Store.createSource(context.session.id, { sourceType: body.sourceType, textContent: text, sourceDigest: digest });
