...
import { shouldShowReanalyze } from "../../../../lib/career/reanalyzeVisibility.mjs";
...
export default function OpportunityDecisionPanel({ opportunityId, initialDecision, stale, reanalyzeRequired = false }: { opportunityId: string; initialDecision: string; stale: boolean; reanalyzeRequired?: boolean }) {
  ...
  const showReanalyze = shouldShowReanalyze({ opportunityId });
  ...
  return <section className="careerProfilePanel"><h2>What do you want to do?</h2><p className="careerMuted">This is your decision about the opportunity. It does not change your career profile or match.</p><div className="careerChoiceGrid">{options.map((option) => <button className="careerSmallButton" key={option.value} aria-pressed={decision === option.value} onClick={() => save(option.value)}>{option.label}</button>)}</div>{message && <p className="careerSaved" role="status">{message}</p>}{stale ? <p className="careerWarning">Your career information has changed since this job was analyzed. Re-analyze the job to refresh the explanation.</p> : reanalyzeRequired ? <p className="careerWarning">This analysis has too little meaningful requirement coverage. Add or review the job information, then re-analyze it.</p> : <p className="careerMuted">{decision === "CONSIDERING" ? "Review the match and decide whether you want to pursue this opportunity." : decision === "PURSUE" ? "This opportunity is marked Pursue. Your analysis is saved." : "This opportunity is marked Pass. You can change your decision later."}</p>}{showReanalyze && <button className="careerPrimaryButton" onClick={reanalyze}>Re-analyze job</button>}</section>;
}
