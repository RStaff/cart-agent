...
export default function MaterialEditor({ savePath, draft: initialDraft, materialType = "RESUME", extraBody = {} }: Props) {
  ...
  async function request(action: string) {
    ...
    setMessage(action === "improve" ? "AI-assisted draft ready for review" : action === "restore" ? "Grounded draft restored" : action === "regenerate" ? "New grounded draft ready for review" : "Draft saved");
  }
  function regenerate() { if (window.confirm("Create a new truthful draft from your current CareerOS evidence? Your existing draft will be preserved.")) request("regenerate"); }
