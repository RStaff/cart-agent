...
type Result = { provider?: string; sourceProvider: string; providerJobId?: string | null; externalOpportunityId: string | null; title: string | null; company: string | null; location: string | null; sourceUrl: string | null; description: string | null; postedAt: string | null; closingAt: string | null; salaryMin: number | null; salaryMax: number | null; employmentType: string | null; retrievedAt: string; recommendation?: string; roleCompatibility?: { classification: string }; discoveryExplanation?: Explanation & { requestedRole?: string; roleAlignment?: string; seniorityAligned?: boolean; specializationAligned?: boolean }; negativeSignals?: string[]; existingState?: string | null };
...
export default function DiscoverClient({ initialPreferences }: { initialPreferences: Preferences }) {
  ...
  async function save(result: Result) {
    ...
    setSaving(key);
    try {
      const response = await fetch("/api/career/opportunity-inbox", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sourceType: "API_IMPORT", sourceName: provider(result), sourceUrl: result.sourceUrl, externalOpportunityId: result.providerJobId || result.externalOpportunityId, discoveredAt: result.retrievedAt, title: result.title, company: result.company, location: result.location, description: result.description }) });
      const body = await response.json();
