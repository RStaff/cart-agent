...
const automaticDiscoveryAvailable = false;
...
export default function DiscoverClient({ initialPreferences }: { initialPreferences: Preferences }) {
  ...
  async function search(event: React.FormEvent) {
    ...
    if (!automaticDiscoveryAvailable) { setResults([]); setMessage(AUTOMATIC_DISCOVERY_UNAVAILABLE_MESSAGE); return; }
    ...
    try {
      ...
      if (!response.ok) { setMessage(body.error === "USAJOBS_WRITTEN_APPROVAL_REQUIRED" ? AUTOMATIC_DISCOVERY_UNAVAILABLE_MESSAGE : body.error || "Automatic discovery is unavailable right now."); return; }
      const statuses = body.existingStatuses || {};
      const resultSource = body.provider === "GREENHOUSE" || body.provider === "SOURCE_REGISTRY" ? "authorized employer sources" : "authorized providers";
  ...
  async function savePreferences() {
    ...
    const body = await response.json(); setSavingPreferences(false); setMessage(response.ok ? "Search preferences saved for future authorized discovery." : body.error || "Search preferences could not be saved.");
  }
