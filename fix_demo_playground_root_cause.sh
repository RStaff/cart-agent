...
echo "🔧 Root-cause fix: use app/demo/playground as the demo route…"

cd ~/projects/cart-agent/abando-frontend

echo "📁 Ensuring app/demo/playground exists…"
mkdir -p app/demo/playground

echo "📝 Writing static, styled demo page to app/demo/playground/page.tsx…"
cat <<'TSX' > app/demo/playground/page.tsx
...
echo "🧹 Removing src/app/demo/playground (to avoid confusion)…"
rm -rf src/app/demo/playground || true

echo "🏗  Building frontend (local sanity check)…"
npm run build

echo "📦 Commit & push…"
cd ~/projects/cart-agent
git add abando-frontend/app/demo/playground/page.tsx
git add -u abando-frontend/src/app/demo/playground || true

git commit -m "Use app/demo/playground static styled demo route" || echo "ℹ️ No changes to commit"
