...
// === Shopify OAuth + Billing (clean, DB-backed) ==========================
...
async function saveShop(domain, accessToken, scopes) {
  const now   = new Date();
...
app.get("/shopify/install", (req, res) => {
  const raw = String(req.query.shop || "").toLowerCase();
  // sanitize to hostname only
  ...
  console.log("[shopify] authorize →", url);
...
app.get("/shopify/callback", async (req, res) => {
  try {
    ...
    if (!shop || !String(shop).toLowerCase().endsWith(".myshopify.com")) {
      return res.status(400).send("Invalid shop");
    }
    if (!code || !hmac || !state || !timestamp) {
      return res.status(400).send("Missing OAuth params");
    }
    if (String(req.cookies?.shopify_state) !== String(state)) {
      return res.status(400).send("State mismatch");
    }

    // Verify HMAC
    const check = { code, shop, state, timestamp };
    const expected = signParams(check);
    ...
    if (!resp.ok) {
      ...
      console.error("[shopify] token exchange failed", resp.status, txt);
      return res.status(500).send("Token exchange failed");
    }
    const { access_token, scope } = await resp.json();

    // Persist in DB
    await saveShop(String(shop).toLowerCase(), access_token, scope);
...
// GET /shopify/billing/start (no-op placeholder; keep your existing logic or stripe flow)
app.get("/shopify/billing/start", async (req, res) => {
  ...
  // If you later wire the Shopify Billing API, do it here. For now just success:
  return res.redirect(`/shopify/billing/return?shop=${encodeURIComponent(shop)}`);
});

// GET /shopify/billing/return
app.get("/shopify/billing/return", (_req, res) => {
  // Minimal success page or redirect to your dashboard
