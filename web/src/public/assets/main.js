
document.addEventListener('DOMContentLoaded', () => {
  // Checkout buttons on pricing page
  document.querySelectorAll('[data-plan]').forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      const plan = btn.getAttribute('data-plan');
      try{
        const res = await fetch('/api/billing/checkout', {
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({ plan })
        });
        const data = await res.json();
        if (data.url) {
          window.location.href = data.url;
        } else {
          alert('Unable to begin checkout: ' + (data.error || 'Unknown error'));
        }
      } catch(err) {
        alert('An error occurred, please try again later.');
      }
    });
  });

  // Playground logic
  const tone   = document.getElementById('tone');
  const offer  = document.getElementById('offer');
  const cta    = document.getElementById('cta');
  const channel= document.getElementById('channel');
  const preview= document.getElementById('preview-message');
  const template = document.getElementById('template');
  const aov    = document.getElementById('aov');
  const ltv    = document.getElementById('ltv');
  const ctr    = document.getElementById('ctr');
  const recovery = document.getElementById('recovery');
  const plan   = document.getElementById('plan');
  const sessions   = document.getElementById('sessions');
  const slider = document.getElementById('sessionSlider');
  const sessionDisplay = document.getElementById('sessionDisplay');
  const kOrders  = document.getElementById('kpi-orders');
  const kRevenue = document.getElementById('kpi-revenue');
  const kROI     = document.getElementById('kpi-roi');
  const nudgeMsg = document.getElementById('nudge-message');

  const templates = {
    custom: {},
    discount: { tone:'friendly', offer:'10% off your next order', cta:'Complete your purchase and save!', channel:'email' },
    urgency:  { tone:'professional', offer:'', cta:'Hurry! Items are almost gone.', channel:'sms' },
    faq:      { tone:'friendly', offer:'', cta:'Got questions? I can help you checkout.', channel:'on-site' }
  };

  function updateMessage() {
    let msg = '';
    const t = tone?.value || 'friendly';
    msg += (t === 'friendly' ? 'Hey there!\n' : t === 'professional' ? 'Hello,\n' : 'Hi,\n');
    msg += 'We noticed you left some items in your cart. ';
    if (offer && offer.value.trim()) msg += 'Here\'s a special offer: ' + offer.value.trim() + '. ';
    msg += 'We\'re here to answer any questions and help you complete your purchase.\n\n';
    msg += (cta && cta.value.trim() ? cta.value.trim() : 'Finish your order') + ' →';
    if (preview) preview.textContent = msg;
  }

  function calcImpact() {
    const s = slider ? Number(slider.value) : (sessions && sessions.value ? Number(sessions.value) : 1000);
    const ctrRate = (ctr && parseFloat(ctr.value)) ? parseFloat(ctr.value) / 100 : 0;
    const recRate = (recovery && parseFloat(recovery.value)) ? parseFloat(recovery.value) / 100 : 0;
    const avg = (ltv && parseFloat(ltv.value) > 0) ? parseFloat(ltv.value) : (aov && parseFloat(aov.value) || 0);
    const pc = (plan && parseFloat(plan.value)) ? parseFloat(plan.value) : 0;
    const orders = s * ctrRate * recRate;
    const revenue = avg > 0 ? orders * avg : NaN;
    const roi = revenue && pc > 0 ? revenue / pc : NaN;
    if (kOrders) kOrders.textContent = Number.isFinite(orders) ? Math.round(orders).toLocaleString() : '—';
    if (kRevenue) kRevenue.textContent = Number.isFinite(revenue) ? ('$' + Math.round(revenue).toLocaleString()) : '—';
    if (kROI) kROI.textContent = Number.isFinite(roi) ? roi.toFixed(1) + '×' : '—';
  }

  // Copy to clipboard
  const copyBtn = document.getElementById('copy');
  copyBtn?.addEventListener('click', () => {
    if (!preview) return;
    navigator.clipboard.writeText(preview.textContent || '').then(() => alert('Message copied')).catch(() => alert('Copy failed'));
  });

  // Bind UI events
  if (tone) tone.addEventListener('input', updateMessage);
  if (offer) offer.addEventListener('input', updateMessage);
  if (cta) cta.addEventListener('input', updateMessage);
  const genBtn = document.getElementById('generate');
  genBtn?.addEventListener('click', updateMessage);

  if (template) {
    template.addEventListener('change', () => {
      const tpl = templates[template.value] || {};
      if (tone && tpl.tone) tone.value = tpl.tone;
      if (channel && tpl.channel) channel.value = tpl.channel;
      if (offer) offer.value = tpl.offer || '';
      if (cta) cta.value   = tpl.cta   || '';
      updateMessage();
    });
  }

  // Sessions slider & number sync
  if (slider) {
    slider.addEventListener('input', () => {
      if (sessions) sessions.value = slider.value;
      if (sessionDisplay) sessionDisplay.textContent = slider.value;
      calcImpact();
    });
  }
  sessions?.addEventListener('input', () => {
    if (slider) slider.value = sessions.value;
    if (sessionDisplay) sessionDisplay.textContent = sessions.value;
    calcImpact();
  });

  // KPI inputs
  aov?.addEventListener('input', calcImpact);
  ltv?.addEventListener('input', calcImpact);
  ctr?.addEventListener('input', calcImpact);
  recovery?.addEventListener('input', calcImpact);
  plan?.addEventListener('input', calcImpact);

  // Nudges rotation
  const nudges = [
    '👍 Acme recovered $4,200 last month with Abando.',
    '🔥 92% open rate for Shopping Copilot messages!',
    '🚀 Merchants recovered an average of $3,800 in their first month.',
    '💬 “Most helpful live chat I’ve seen” — Beta user'
  ];
  let nIdx = 0;
  function rotateNudge() {
    if (nudgeMsg) {
      nudgeMsg.textContent = nudges[nIdx];
      nIdx = (nIdx + 1) % nudges.length;
    }
  }
  setInterval(rotateNudge, 5000);
  rotateNudge();

  // Initial render
  updateMessage();
  calcImpact();
});
