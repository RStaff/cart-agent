...
<section class="section"><div class="container">
  <div class="card" id="shopify-install-banner" style="display:none;align-items:center;justify-content:space-between;gap:.75rem">
    ...
    <form method="GET"
      action="https://pay.abando.ai/api/auth"
      target="_top"
      onsubmit="
        try {
          var qs = new URLSearchParams(window.location.search || '');
          var shop = (qs.get('shop') || '').trim();
          var hidden = this.querySelector('input[name=shop]');
          if (hidden) hidden.value = shop;
        } catch (e) {}
      "
      style="margin:0; display:inline-block;">
      <input type="hidden" name="shop" value="">
      <button type="submit" class="btn btn-primary" data-install-shopify-btn>
        Install via Shopify
      </button>
    </form>
  </div>
