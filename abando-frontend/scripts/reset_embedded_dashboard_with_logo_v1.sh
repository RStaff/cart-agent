...
BACKUP="${FILE}.before_logo_reset_$(date +%s).tsx"

echo "📁 Repo: $(pwd)"
echo "💾 Backing up current embedded page to: $BACKUP"
cp "$FILE" "$BACKUP"
        ...
        <header className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              {/* Inline Abando glyph — always renders, no asset dependency */}
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-500/10 ring-1 ring-emerald-500/40 shadow-[0_0_0_1px_rgba(16,185,129,0.35)]">
                <span className="text-sm font-semibold text-emerald-300">
                  A
                </span>
              </div>
              <p className="text-[0.65rem] font-semibold uppercase tracking-[0.24em] text-emerald-400">
                Abando dashboard · Live view of recovered orders &amp; shopper
                patterns
              </p>
            </div>
          ...
          <div className="flex flex-col items-start gap-3 text-right md:items-end">
            ...
            <button className="inline-flex items-center rounded-full border border-emerald-500/60 bg-emerald-500/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-200 shadow-[0_0_0_1px_rgba(16,185,129,0.35)] hover:bg-emerald-500/15">
...
echo "✅ app/embedded/page.tsx reset with Abando logo + working pills + Abando response"
