...
echo "▶ [CC v2] Moving to project root..."
cd "$(dirname "$0")/.."
echo "   Project root: $(pwd)"

# 1) Remove duplicate JS page if it exists
DUP="src/app/command-center/page.js"
if [ -f "$DUP" ]; then
  echo "▶ [CC v2] Removing duplicate route file: $DUP"
  rm "$DUP"
  echo "   ✅ page.js removed."
else
  echo "   ✅ No page.js duplicate found (good)."
fi

# 2) Write StatusPanel (dashboard-style layout)
echo "▶ [CC v2] Writing src/app/command-center/StatusPanel.tsx ..."
cat << 'CC_STATUS' > src/app/command-center/StatusPanel.tsx
const summaryCards = [
  {
...
# 3) Write page.tsx shell
echo "▶ [CC v2] Writing src/app/command-center/page.tsx ..."
cat << 'CC_PAGE' > src/app/command-center/page.tsx
...
echo "✅ Command Center v2 dashboard files written."
echo "Next steps:"
echo "  1) npm run dev"
echo "  2) Visit http://localhost:3000/command-center"
echo "  3) When satisfied visually: ./scripts/deploy_abando_frontend.sh && ~/abando_quality_check.sh"
