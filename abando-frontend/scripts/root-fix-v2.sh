...
ROOT="$HOME/projects/cart-agent/abando-frontend"
TS=$(date +%Y%m%d-%H%M%S)

echo "→ Root-fix for /v2 (backup tag $TS)"
test -d "$APP/src/app" || { echo "❌ $APP/src/app not found"; exit 1; }

# 0) Tailwind/PostCSS at WORKSPACE ROOT (Next resolves here)
cd "$ROOT"
...
module.exports = {
  content: [
    "./abando-frontend/src/**/*.{ts,tsx,mdx}",
    "./abando-frontend/src/app/**/*.{ts,tsx,mdx}",
    "./abando-frontend/src/components/**/*.{ts,tsx,mdx}",
  ],
  theme: { extend: {} },
  plugins: [],
};
JS
  echo "✅ wrote $ROOT/tailwind.config.js"
else
  CFG=$(ls -1 tailwind.config.* | head -n1)
  if ! grep -q 'abando-frontend/src' "$CFG"; then
    perl -0777 -i -pe 's/content:\s*\[/content: [\n  ".\/abando-frontend\/src\/**\/*.{ts,tsx,mdx}",\n  ".\/abando-frontend\/src\/app\/**\/*.{ts,tsx,mdx}",\n  ".\/abando-frontend\/src\/components\/**\/*.{ts,tsx,mdx}",\n/s' "$CFG" || true
    echo "✅ ensured app globs in $CFG"
  else
    echo "ℹ️ tailwind content globs already include app paths"
  fi
fi
...
JS
  echo "✅ wrote $ROOT/postcss.config.js"
else
  echo "ℹ️ $ROOT/postcss.config.js exists"
fi

# 1) Globals: keep global rules out of CSS modules
cd "$APP"
...
CSS
    ...
    echo "ℹ️ globals.css already has base tokens"
  fi
else
  echo "⚠️ src/app/globals.css missing (continuing)"
fi

# 2) public logo: keep if present, otherwise create placeholder
mkdir -p public
...
# 3) Clean /v2 components & styles (with backups)
mkdir -p src/app/v2
[ -f src/app/v2/page.tsx ] && cp src/app/v2/page.tsx "src/app/v2/page.tsx.bak.$TS"
[ -f src/app/v2/page.module.css ] && cp src/app/v2/page.module.css "src/app/v2/page.module.css.bak.$TS"
[ -f src/app/v2/NavBar.tsx ] && cp src/app/v2/NavBar.tsx "src/app/v2/NavBar.tsx.bak.$TS"
[ -f src/app/v2/Brand.tsx ] && cp src/app/v2/Brand.tsx "src/app/v2/Brand.tsx.bak.$TS"
...
export default function Brand() {
  return (
    <Link href="/" className={styles.brand} aria-label="Abando home">
      {/* decorative icon only; alt="" avoids redundant text */}
      <Image
        src="/abando-logo.svg"
        alt=""
        width={28}
      ...
      <section className={styles.hero}>
        <div className={styles.container}>
          ...
          <p className={styles.sub}>
            Abando<sup>™</sup> answers questions, handles objections, and guides buyers through checkout—so abandonment turns into orders.
          </p>
...
/* brand + nav (locked sticky, production feel) */
