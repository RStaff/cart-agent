...
export function rateLimit(key: string, opts = { capacity: 12, refillPerSec: 3 }): boolean {
  const now = Date.now();
  ...
  if (b.tokens < 1) { buckets.set(key, { ...b, ts: now }); return false; }
  b.tokens -= 1; b.ts = now; buckets.set(key, b); return true;
}
