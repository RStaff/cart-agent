...
export default function Explainer(props: PropsModern | PropsLegacy) {
  ...
  if ("storageKey" in props) {
    ...
    React.useEffect(() => {
      if (typeof window !== "undefined" && shouldShowFromKey(storageKey))
        setOpen(true);
    }, [storageKey]);
...
function ExplainerInner({
  ...
  React.useEffect(() => {
    if (open) fire("explainer_shown", { page });
  }, [open, page]);
  if (!open) return null;

  const title =
    page === "demo" ? "See it work in 30 seconds" : "Turn demos into revenue";
  const primary = page === "demo" ? "Start the 30-second demo" : "Open Demo";
  const points =
    page === "demo"
      ? [
          "Describe your shopper (persona + concerns).",
          "Pick a voice or write your own tone.",
          "Compare variants — ship the winner.",
        ]
      : [
          "Your latest demo result appears here.",
          "Variants are saved so teammates can review.",
          "Connect your store to recover carts automatically.",
        ];
  ...
  return (
    ...
    >
      ...
      >
        <h2 id="explainer-title" style={{ margin: "0 0 8px 0" }}>
          {title}
        </h2>
