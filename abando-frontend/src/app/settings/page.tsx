...
export default function SettingsPage() {
  const [status, setStatus] = React.useState<{ok:boolean; env:{hasPublishable:boolean; hasSecret:boolean; publishablePreview:string}}|null>(null);
      ...
      <div className={`rounded-lg border p-5 mb-6 ${ok ? "border-emerald-700 bg-emerald-950" : "border-amber-700 bg-slate-800"}`}>
        ...
        {status && (
          <ul className="text-slate-300 list-disc ml-5 space-y-1">
            <li>Publishable key present: <b>{status.env.hasPublishable ? "Yes" : "No"}</b> {status.env.publishablePreview && `(${status.env.publishablePreview})`}</li>
            <li>Secret key present: <b>{status.env.hasSecret ? "Yes" : "No"}</b></li>
          </ul>
        )}
...
STRIPE_SECRET_KEY=sk_live_xxx
      ...
      <div className="rounded-lg border border-slate-800 bg-slate-950 p-5">
        ...
        <ol className="text-slate-300 list-decimal ml-5 space-y-1">
          <li>Set the keys in <code>.env.local</code>.</li>
