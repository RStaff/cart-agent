...
# Prefer a dev script, otherwise use npx next
if node -e 'const p=require("./abando-frontend/package.json"); process.exit(p.scripts&&p.scripts.dev?0:1)'; then
  nohup npm --prefix abando-frontend run dev -- --port 3001 > .dev_next.log 2>&1 &
else
  nohup bash -lc 'cd abando-frontend && npx next dev -p 3001' > .dev_next.log 2>&1 &
fi
NEXT_PID=$!
...
echo "⏳ Waiting for Express healthz..."
