...
echo "📄 Overwriting api/server.js with unified handlers + /api/ai-segments…"

cat > api/server.js << 'NODE'
"use strict";
...
// Postgres pool for summary endpoints
const pool = new Pool({
...
// -----------------------------------------------------------------------------
// AI segments summary endpoint
// GET /api/ai-segments/:storeId
// -----------------------------------------------------------------------------
app.get("/api/ai-segments/:storeId", async (req, res) => {
  ...
  try {
    ...
    const { rows: summary } = await pool.query(summarySql, [storeId]);
    const { rows: recent } = await pool.query(recentSql, [storeId]);
...
echo "✅ api/server.js overwritten with unified handlers + /api/ai-segments"
