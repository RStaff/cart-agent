...
if (isESM) {
  fs.writeFileSync('src/dev/checkoutPublic.esm.js', `import express from "express";
...
`);
  ...
  fs.writeFileSync('src/dev/checkoutPublic.cjs', `const express = require("express");
...
// Patch entry: add import/require, ensure express.json(), and mount before any legacy handler.
