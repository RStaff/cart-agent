...
# Router (status, __align, robots, sitemap)
...
# Ensure "start" preloads shim (works on Render)
node - <<'JS'
const fs = require('fs');
const pkg = JSON.parse(fs.readFileSync('package.json','utf8'));
pkg.scripts = pkg.scripts || {};
const need = 'node --require ./web/smc-preload.cjs';
let start = pkg.scripts.start || "node web/src/index.js";
if (!start.includes('--require ./web/smc-preload.cjs')) {
  // normalize: convert "node xyz" to "node --require ./web/smc-preload.cjs xyz"
  start = start.replace(/^node\s+/, need + ' ');
...
git add -A
git commit -m "abando: preload shim installs alignment router regardless of entry; add /api/status & SEO" || true
